...
const TAG_COLORS: Record<string, { bg: string; text: string; darkBg: string; darkText: string }> = {
  Laravel:        { bg: "#FEE2E2", text: "#B91C1C", darkBg: "#3B1212", darkText: "#FCA5A5" },
  PHP:            { bg: "#EDE9FE", text: "#6D28D9", darkBg: "#2D1F4E", darkText: "#C4B5FD" },
  MySQL:          { bg: "#DBEAFE", text: "#1D4ED8", darkBg: "#172554", darkText: "#93C5FD" },
  JavaScript:     { bg: "#FEF9C3", text: "#92400E", darkBg: "#3B2900", darkText: "#FDE68A" },
  jQuery:         { bg: "#E0F2FE", text: "#0369A1", darkBg: "#0C1F33", darkText: "#7DD3FC" },
  Bootstrap:      { bg: "#EDE9FE", text: "#7C3AED", darkBg: "#2D1B69", darkText: "#DDD6FE" },
  Git:            { bg: "#FFE4E1", text: "#C0392B", darkBg: "#3B1010", darkText: "#FCA5A5" },
  GitHub:         { bg: "#F3F4F6", text: "#1F2937", darkBg: "#1F2937", darkText: "#D1D5DB" },
  "Node.js":      { bg: "#DCFCE7", text: "#15803D", darkBg: "#052E16", darkText: "#86EFAC" },
  "GitHub REST API": { bg: "#F3F4F6", text: "#374151", darkBg: "#1F2937", darkText: "#D1D5DB" },
  HTML5:          { bg: "#FFE4E1", text: "#C2410C", darkBg: "#3B1200", darkText: "#FDBA74" },
  CSS3:           { bg: "#DBEAFE", text: "#1E40AF", darkBg: "#172554", darkText: "#93C5FD" },
  "Local Storage":{ bg: "#ECFDF5", text: "#065F46", darkBg: "#052E16", darkText: "#6EE7B7" },
  JSON:           { bg: "#FEF9C3", text: "#78350F", darkBg: "#3B2000", darkText: "#FDE68A" },
};

const DEFAULT_TAG = { bg: "#F3F4F6", text: "#374151", darkBg: "#374151", darkText: "#D1D5DB" };
...
const projects = [
  {
    ...
    image: project1,
    accentColor: "#8B5CF6",
    tags: ["Laravel", "PHP", "MySQL", "JavaScript", "jQuery", "Bootstrap", "Git", "GitHub"],
    features: [
      "Role-Based Authentication", "Inventory Management",
      "Billing & Invoice Generation", "Employee Management",
      "Customer Management", "Sales Tracking",
      "Reporting Dashboard", "Automatic Stock Updates",
    ],
    credentials: [
      { role: "Admin",    email: "admin@gmail.com",    password: "12345678" },
      { role: "Employee", email: "employee@gmail.com", password: "12345678" },
    ],
  ...
  },
  {
    ...
    features: [
      ...
      "Real-Time UI Updates", "Responsive Design", "Local Storage Persistence",
    ],
    credentials: [],
    github: null,
  ...
  },
  {
    ...
    features: [
      "API Integration", "Async/Await", "Event Tracking",
      "JSON Processing", "Error Handling", "Input Validation",
    ],
    credentials: [],
...
const ProjectCard = ({ project, index, onClick }: { project: Project; index: number; onClick: () => void }) => {
  ...
  return (
    ...
    >
      {/* Accent top bar */}
      <div className="portfolio-card-bar" style={{ background: project.accentColor }} />
    ...
    <section id="portfolio" className="portfolio-section py-28">
      ...
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="portfolio-dialog max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl">
          {selected && (
            <>
              <DialogTitle className="text-2xl font-bold mb-1">{selected.title}</DialogTitle>
              <p className="text-xs font-semibold uppercase tracking-wide mb-4" style={{ color: selected.accentColor }}>
                {selected.category}
              </p>
              <img src={selected.image} alt={selected.title} className="w-full h-64 object-cover rounded-2xl mb-6" />
              ...
              <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {selected.credentials.map(c => (
                        <div key={c.role} className="portfolio-cred-card rounded-2xl p-4 space-y-1">
                          <p className="text-sm font-bold" style={{ color: selected.accentColor }}>{c.role}</p>
                          <p className="text-sm text-muted-foreground"><span className="font-medium text-foreground">Email:</span> {c.email}</p>
                          <p className="text-sm text-muted-foreground"><span className="font-medium text-foreground">Password:</span> {c.password}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <ul className="grid grid-cols-2 gap-1.5">
                    {selected.features.map(f => (
                      <li key={f} className="text-sm text-muted-foreground flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full inline-block flex-shrink-0" style={{ background: selected.accentColor }} />
                        {f}
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selected.tags.map(tag => <TagChip key={tag} tag={tag} large />)}
                  </div>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  {selected.demo && (
                    <Button className="flex-1 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground" size="lg"
